		...
		if (*list != NULL) {
			...
			for (idx = 0; idx < MAX_CONN; idx++, list++) {
				if (*list) { /* process connection name */
					...
					// leftsubnet
					buf[0] = '\0';
					if (get_ipsec_subnet(LOCAL, *list, buf)
					                > 0) {
						if (strlen(buf) > 0)
							fprintf(
							                out,
					...
					// leftnexthop
					buf[0] = '\0';
					if (get_ipsec_nexthop(LOCAL, *list, buf)
					                > 0) {
						if (strlen(buf) > 0)
							fprintf(
							                out,
					...
					// rightid
					buf[0] = '\0';
					if (get_ipsec_id(REMOTE, *list, buf)
					                > 0) {
						if (strlen(buf) > 0)
							fprintf(
							                out,
					...
					// right address
					buf[0] = '\0';
					switch (get_ipsec_remote_addr(*list,
					                buf)) {
					case ADDR_ANY:
						fprintf(out,
						                "  remote address any\n");
					...
					// rightsubnet
					buf[0] = '\0';
					if (get_ipsec_subnet(REMOTE, *list, buf)
					                > 0) {
						if (strlen(buf) > 0)
							fprintf(
							                out,
					...
					// rightnexthop
					buf[0] = '\0';
					if (get_ipsec_nexthop(REMOTE, *list,
					                buf) > 0) {
						if (strlen(buf) > 0)
							fprintf(
							                out,
					...
					// rightrsasigkey
					pt = NULL;
					if (get_ipsec_rsakey(*list,
					                STRING_IPSEC_R_RSAKEY,
					                &pt) > 0) {
						if (pt) {
							fprintf(
							                out,
					...
					if ((pt = get_ipsec_protoport(*list))
					                != NULL) {
						fprintf(
						                out,
						                "  l2tp protoport %s\n",
