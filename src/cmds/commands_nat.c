		 ...
		 char *masq_ports)
{
	...
	if (conf_format) fprintf (out, "nat-rule ");
	if (conf_format) fprintf (out, "%s ", acl);
		else fprintf (out, "    ");
	if (strcmp (proto, "all") == 0) fprintf (out, "ip ");
		else fprintf (out, "%s ", proto);
	if (strcasecmp (src, "0.0.0.0/0") == 0) fprintf (out, "any ");
		else if (strcmp (src_netmask, "255.255.255.255") == 0) fprintf (out, "host %s ", _src);
			else fprintf (out, "%s %s ", _src, ciscomask(src_netmask));
	if (*src_ports) fprintf (out, "%s ", src_ports);
	if (strcasecmp (dst, "0.0.0.0/0") == 0) fprintf (out, "any ");
		else if (strcmp (dst_netmask, "255.255.255.255") == 0) fprintf (out, "host %s ", _dst);
			else fprintf (out, "%s %s ", _dst, ciscomask(dst_netmask));
	if (*dst_ports) fprintf (out, "%s ", dst_ports);
	if (to)
	{
		char *p;
	...
	if (!conf_format) fprintf (out, " (%i matches)", mc);
	fprintf (out, "\n");
}
			...
			if ((strlen(buf)) && ((xacl == NULL) || (strcmp(xacl,acl) == 0)))
			{
				...
				else
				{
					if (!conf_format)
					{
						...
						{
							if (strcmp(input, "*")) fprintf(F, "interface %s in nat-rule %s\n", input, type);
							if (strcmp(output, "*")) fprintf(F, "interface %s out nat-rule %s\n", output, type);
						}
					}
				}
				destroy_args(args);
