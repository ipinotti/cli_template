...
void do_accesslist(const char *cmdline)
{
	...
	if (acl.protocol == icmp) {
		if (strcmp (args->argv[crsr], "type") == 0) {
			...
			++crsr;
			if (strcmp (acl.icmp_type, "destination-unreachable") == 0
			                || strcmp (acl.icmp_type, "redirect") == 0
			                || strcmp (acl.icmp_type, "time-exceeded") == 0
			                || strcmp (acl.icmp_type, "parameter-problem") == 0) {
				if (crsr >= args->argc) {
