...
static void refresh_dynamic_ipsec_menus(void)
{
	...
	for(i=CMDS_BEF_LIST; i < (MAX_CONN+CMDS_BEF_LIST); i++)
	{
		if(!CMD_IPSEC_CONNECTION_ADD[i].name && CMD_IPSEC_CONNECTION_ADD[i+1].name)
		{
			...
			CMD_IPSEC_CONNECTION_ADD[i].children = CMD_IPSEC_CONNECTION_ADD[i+1].children;
			CMD_IPSEC_CONNECTION_ADD[i].func = CMD_IPSEC_CONNECTION_ADD[i+1].func;
			CMD_IPSEC_CONNECTION_ADD[i].privilege = CMD_IPSEC_CONNECTION_ADD[i+1].privilege;
			...
			CMD_CRYPTO_IPSEC_NO_CONN[j].children = CMD_CRYPTO_IPSEC_NO_CONN[j+1].children;
			CMD_CRYPTO_IPSEC_NO_CONN[j].func = CMD_CRYPTO_IPSEC_NO_CONN[j+1].func;
			CMD_CRYPTO_IPSEC_NO_CONN[j].privilege = CMD_CRYPTO_IPSEC_NO_CONN[j+1].privilege;
...
int eval_connections_menus(int add_del, char *name)
{
	...
	{	// add name
		for(i=CMDS_BEF_LIST; i < (MAX_CONN+CMDS_BEF_LIST); i++)
		{
			if (CMD_IPSEC_CONNECTION_ADD[i].name == NULL) break;
		}
		if (i >= (MAX_CONN+CMDS_BEF_LIST)) return -1;
		if ((p=malloc(strlen(name)+1)) == NULL) return -1;
		strcpy(p, name);
...
void ipsec_autoreload(const char *cmd) /* [no] auto-reload [60-3600] */
{
	...
	args=make_args(cmd);
	if (strcmp(args->argv[0], "no") == 0) replace_string_in_file_nl(FILE_IPSEC_CONF, STRING_IPSEC_AUTORELOAD, "0");
		else replace_string_in_file_nl(FILE_IPSEC_CONF, STRING_IPSEC_AUTORELOAD, args->argv[1]);
	destroy_args(args);
}
...
void ipsec_nat_traversal(const char *cmd) /* [no] nat-traversal */
{
	...
	args=make_args(cmd);
	if (args->argc == 2) replace_string_in_file_nl(FILE_IPSEC_CONF, STRING_IPSEC_NAT, "no");
		else replace_string_in_file_nl(FILE_IPSEC_CONF, STRING_IPSEC_NAT, "yes");
	destroy_args(args);
}
...
void ipsec_overridemtu(const char *cmd) /* [no] overridemtu [64-1460] */
{
	...
	args=make_args(cmd);
	if (strcmp(args->argv[0], "no") == 0) replace_string_in_file_nl(FILE_IPSEC_CONF, STRING_IPSEC_OMTU, "0");
		else replace_string_in_file_nl(FILE_IPSEC_CONF, STRING_IPSEC_OMTU, args->argv[1]);
	destroy_args(args);
}
...
void add_ipsec_conn(const char *cmd) /* ipsec connection add [name] */
{
	...
	if (args->argc == 4)
	{
		...
		if (*list != NULL)
		{
			...
			{
				if (strcmp(args->argv[3], *list) == 0) go_out++;
				free(*list);
			}
			free(list_ini);
			if (go_out)
			{
				if (!_cish_booting) printf ("%% Connection with name %s already exists!\n", args->argv[3]);
				goto free_args;
			}
...
void del_ipsec_conn(const char *cmd) /* no ipsec connection [name] */
{
	...
	if (args->argc == 4)
	{
		...
		if (eval_connections_menus(0, args->argv[3]) < 0) goto free_args;
		...
		if (list_all_ipsec_names(&list) > 0)
		{
			if (*list != NULL)
			{
				...
				for (i=0; i < MAX_CONN; i++, list++)
				{
					if (*list)
					{
						if (get_ipsec_link(*list)) restart=1;
						free(*list);
					}
				}
				free(list_ini);
		...
		if (*list != NULL)
		{
			...
			for (i=0; i < MAX_CONN; i++, list++)
			{
				if (*list)
				{
					...
					if (ret == RSA) create_secrets_conn_file(*list, 1, NULL);
					free(*list);
				}
			}
			free(list_ini);
...
void ipsec_set_secret_key(const char *cmd) /* authby secret password */
{
	...
	if(args->argc == 3)
	{
		if (create_secrets_conn_file(dynamic_ipsec_menu_name, 0, args->argv[2]) < 0)
		{
...
void ipsec_authby_rsa(const char *cmd)
{
	...
	if(args->argc == 2)
	{
		if (create_secrets_conn_file(dynamic_ipsec_menu_name, 1, NULL) < 0)
		{
...
void set_esp_hash(const char *cmd)
{
	...
	switch(args->argc)
	{
		case 1:
			...
			break;
		case 2:
			if(set_ipsec_esp(dynamic_ipsec_menu_name, args->argv[1], NULL) < 0)
			{
			...
			break;
		case 3:
			if(set_ipsec_esp(dynamic_ipsec_menu_name, args->argv[1], args->argv[2]) < 0)
			{
...
void set_ipsec_id(const char *cmd)
{
	...
	if (args->argc == 3)
	{
		if (strlen(args->argv[2]) < MAX_ID_LEN) strcpy(tp, args->argv[2]);
		else
		{
			printf("%% ID to long\n");
			goto free_args;
		}
		if (strncmp(args->argv[0], "local", 5) == 0) ret=set_ipsec_local_id(dynamic_ipsec_menu_name, tp);
			else ret=set_ipsec_remote_id(dynamic_ipsec_menu_name, tp);
		if (ret < 0)
		{
...
void clear_ipsec_id(const char *cmd) /* no local/remote id */
{
	...
	if (args->argc == 3)
	{
		if (strncmp(args->argv[1], "local", 5) == 0) ret=set_ipsec_local_id(dynamic_ipsec_menu_name, "");
			else ret=set_ipsec_remote_id(dynamic_ipsec_menu_name, "");
		if (ret < 0)
		{
...
void set_ipsec_addr(const char *cmd) /* local/remote address default-route/interface/ip/fqdn serial/x.x.x.x/www */
{
	...
	if(args->argc >= 3)
	{
		if (!strncmp(args->argv[0], "local", 5)) local=1;
		if (local && !strncmp(args->argv[2], "default-route", 13))
		{
			ret=set_ipsec_local_addr(dynamic_ipsec_menu_name, STRING_DEFAULTROUTE);
			if (ret < 0)
			{
		...
		{
			ret=set_ipsec_remote_addr(dynamic_ipsec_menu_name, STRING_ANY);
			if (ret < 0)
			{
		...
		else if (args->argc == 4 && (!strncmp(args->argv[2], "ip", 2) || !strncmp(args->argv[2], "fqdn", 4)))
		{
			if (strlen(args->argv[3]) < 200) strcpy(tp, args->argv[3]);
			else
			{
				printf("%% Not possible to set %s address\n", args->argv[0]);
				goto free_args;
			}
			if (local) ret=set_ipsec_local_addr(dynamic_ipsec_menu_name, tp);
				else ret=set_ipsec_remote_addr(dynamic_ipsec_menu_name, tp);
			if (ret < 0)
			{
				if (!strncmp(args->argv[2], "ip", 2)) printf("%% Not possible to set %s ip address\n", args->argv[0]);
					else printf("%% Not possible to set %s fqdn address\n", args->argv[0]);
				goto free_args;
			}
		}
...
void set_ipsec_nexthop(const char *cmd)
{
	...
	if (args->argc == 3)
	{
		if (!strncmp(args->argv[0], "local", 5)) ret=set_ipsec_nexthop_inf(LOCAL, dynamic_ipsec_menu_name, args->argv[2]);
			else ret=set_ipsec_nexthop_inf(REMOTE, dynamic_ipsec_menu_name, args->argv[2]);
		if (ret < 0)
		{
...
void clear_ipsec_nexthop(const char *cmd) /* no local/remote nexthop */
{
	...
	if (args->argc == 3)
	{
		if (!strncmp(args->argv[1], "local", 5)) ret=set_ipsec_nexthop_inf(LOCAL, dynamic_ipsec_menu_name, "");
			else ret=set_ipsec_nexthop_inf(REMOTE, dynamic_ipsec_menu_name, "");
		if (ret < 0)
		{
...
void set_ipsec_remote_rsakey(const char *cmd) /* remote rsakey [publickey] */
{
	...
	if (args->argc == 3)
	{
		if (set_ipsec_rsakey(dynamic_ipsec_menu_name, STRING_IPSEC_R_RSAKEY, args->argv[2]) < 0)
		{
...
void clear_ipsec_remote_rsakey(const char *cmd) /* no local/remote rsakey */
{
	...
	if (args->argc == 3)
	{
		if (set_ipsec_rsakey(dynamic_ipsec_menu_name, STRING_IPSEC_R_RSAKEY, "") < 0)
		{
...
void set_ipsec_subnet(const char *cmd)
{
	...
	if (args->argc == 4)
	{
		if (!strncmp(args->argv[0], "local", 5)) ret=set_ipsec_subnet_inf(LOCAL, dynamic_ipsec_menu_name, args->argv[2], args->argv[3]);
			else ret=set_ipsec_subnet_inf(REMOTE, dynamic_ipsec_menu_name, args->argv[2], args->argv[3]);
		if (ret < 0)
		{
...
void clear_ipsec_subnet(const char *cmd) /* no local/remote subnet */
{
	...
	if (args->argc == 3)
	{
		if (!strncmp(args->argv[1], "local", 5)) ret=set_ipsec_subnet_inf(LOCAL, dynamic_ipsec_menu_name, "", "");
			else ret=set_ipsec_subnet_inf(REMOTE, dynamic_ipsec_menu_name, "", "");
		if (ret < 0)
		{
...
void ipsec_link_up(const char *cmd)
{
	...
	if (get_ipsec()) set_ipsec(RESTART);
		else set_ipsec(START);
}
...
void set_ipsec_l2tp_protoport(const char *cmd) /* [no] l2tp protoport [SP1|SP2] */
{
	...
	if (args->argc == 3)
	{
		if (!strcmp(args->argv[0], "no")) set_ipsec_protoport(dynamic_ipsec_menu_name, NULL);
			else set_ipsec_protoport(dynamic_ipsec_menu_name, args->argv[2]);
		// se o link estiver up, entao provocamos um RESTART no starter
		ret=get_ipsec_link(dynamic_ipsec_menu_name);
		if(ret < 0)
		{
...
void l2tp_server(const char *cmd) /* [no] l2tp server */
{
	...
	args=make_args(cmd);
	if (args->argc == 3 && !strcmp(args->argv[0], "no")) set_l2tp(STOP);
		else set_l2tp(START);
	destroy_args(args);
}
...
void check_initial_conn(void)
{
	...
	if (list_all_ipsec_names(&list) >= 0)
	{
		if (*list != NULL)
		{
			...
			for(i=0; i < MAX_CONN; i++)
			{
				if (*list != NULL)
				{
					for(j=CMDS_BEF_LIST; ; j++)
					{
						...
						{
							if ((p=malloc(strlen(*list)+1)) == NULL) break;
							strcpy(p, *list);
							list++;
							CMD_IPSEC_CONNECTION_ADD[j].name = p;
							CMD_IPSEC_CONNECTION_ADD[j].help = "User connection";
							CMD_IPSEC_CONNECTION_ADD[j].func = cd_connection_dir;
							CMD_IPSEC_CONNECTION_ADD[j].privilege = 1;
							// prepare del node
							CMD_CRYPTO_IPSEC_NO_CONN[j-CMDS_BEF_LIST].name = p;
							CMD_CRYPTO_IPSEC_NO_CONN[j-CMDS_BEF_LIST].help = "User connection";
							CMD_CRYPTO_IPSEC_NO_CONN[j-CMDS_BEF_LIST].func = del_ipsec_conn;
							CMD_CRYPTO_IPSEC_NO_CONN[j-CMDS_BEF_LIST].privilege = 1;
							break;
						}
					}
				}
			...
			// Libera memoria
			for (i=0, list=list_ini; i < MAX_CONN; i++, list++) if (*list) free(*list);
			free(list_ini);
		}
	}
}
...
void l2tp_ppp_peeraddr(const char *cmd) /* l2tp ppp ip peer-address [pool|<ipaddress>] */
{
	...
	args=make_args(cmd);
	l2tp_ppp_get_config(dynamic_ipsec_menu_name, &cfg);
	if (strcmp(args->argv[4], "pool") == 0) cfg.ip_peer_addr[0]=0;
		else { strncpy(cfg.ip_peer_addr, args->argv[4], 16); cfg.ip_peer_addr[15]=0; }
	l2tp_ppp_set_config(dynamic_ipsec_menu_name, &cfg);
	destroy_args(args);
}
