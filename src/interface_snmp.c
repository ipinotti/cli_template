...
cish_command CMD_CONFIG_NO_SNMP[] = {
	...
	{"<enter>", "Disable SNMP server", NULL, NULL, 1, MSK_NORMAL},
	{NULL, NULL, NULL, NULL, 0}
};
...
void snmp_community (const char *cmd)
{
	...
	if ((args->argc>=4)&&(strcasecmp(args->argv[3], "rw")==0)) ro=0;
		else ro=1;
...
void snmp_no_community (const char *cmd)
{
	...
	if ((args->argc>=5)&&(strcasecmp(args->argv[4], "rw")==0)) ro=0;
		else ro=1;
...
void snmp_trapsink(const char *cmd) /* snmp trapsink <ipaddress> <community> */
{
	...
	args=make_args(cmd);
	if(args->argc >= 4)	snmp_add_trapsink(args->argv[2], args->argv[3]);
	destroy_args(args);
}
...
void snmp_no_trapsink(const char *cmd) /* no snmp trapsink <ipaddress> */
{
	...
	args=make_args(cmd);
	if (args->argc == 4) snmp_del_trapsink(args->argv[3]);
	destroy_args(args);
}
...
void snmp_user(const char *cmd) /* [no] snmp-server user <username> <rw | ro> <authpriv type> [authproto <md5 | sha>] [privproto <des | aes>] */
{
	...
	switch( strcmp(args->argv[0], "no") ) {
		...
		default: /* Adicao de usuario */
			switch( args->argc ) {
				case 5:
					if( add_snmp_user(args->argv[2], ((strcmp(args->argv[3], "rw") == 0) ? 1 : 0), args->argv[4], NULL, NULL, NULL, NULL) < 0 )
				...
				case 7:
					...
					if( add_snmp_user(args->argv[2], ((strcmp(args->argv[3], "rw") == 0) ? 1 : 0), args->argv[4], args->argv[6], NULL, authpasswd, NULL) < 0 )
						printf("%% Not possible to add user\n");
					break;
				...
				case 9:
					...
					if( add_snmp_user(args->argv[2], ((strcmp(args->argv[3], "rw") == 0) ? 1 : 0), args->argv[4], args->argv[6], args->argv[8], authpasswd, privpasswd) < 0 )
						printf("%% Not possible to add user\n");
					break;
	...
	if( (f = fopen(SNMP_USERKEY_FILE, "r")) != NULL ) {
		while( feof(f) == 0 ) {
			if( fgets(buf, 255, f) != NULL ) {
				buf[255] = 0;
				if( (n = parse_args_din(buf, &argl)) >= 2 ) {
