...
char *hash_str(unsigned char *buf, unsigned char id)
{
	int i;
	unsigned char md5buffer[16];
	unsigned char buffer2[8];
	static unsigned char password[17];
	...
	md5_buffer((char *)buf, 32, md5buffer);
#else
	...
	buffer2[5] += 0x59;
	buffer2[4] += 0x06;
	for (i = 0; i < 8; i++) sprintf((char *)password+(i<<1), "%02x", buffer2[i]);
	password[16]=0;
