...
void cmd_aaa_authen(const char *cmd) 
{
	...
	if((exec_line_args_len = parse_args_din((char *) cmd, &exec_line_args))) {
		...
		if (!strcmp(exec_line_args[0], "no") || !strcmp(exec_line_args[4], "none")) {
			if (!strcmp(exec_line_args[3], "login") || !strcmp(exec_line_args[2], "login")) {
				if(!conf_pam_mode(cish_cfg, AAA_AUTH_NONE, 1, FILE_PAM_GENERIC)) {
					printf("%% Not possible to execute command with success\n");
					free_args_din(&exec_line_args);
					return;
				}
			} else if (!strcmp(exec_line_args[3], "ppp") || !strcmp(exec_line_args[2], "ppp")) {
				if(!conf_pam_mode(cish_cfg, AAA_AUTH_NONE, 1, FILE_PAM_PPP)) {
					printf("%% Not possible to execute command with success\n");
					free_args_din(&exec_line_args);
					return;
				}
			...
			if (!strcmp(exec_line_args[2], "login")) {
				if(!conf_pam_mode(cish_cfg, AAA_AUTH_LOCAL, 1, FILE_PAM_GENERIC))	{
					printf("%% Not possible to execute command with success\n");
					free_args_din(&exec_line_args);
					return;
				}
			} else if (!strcmp(exec_line_args[2], "ppp")) {
				if(!conf_pam_mode(cish_cfg, AAA_AUTH_LOCAL, 1, FILE_PAM_PPP))	{
					printf("%% Not possible to execute command with success\n");
					free_args_din(&exec_line_args);
					return;
				}
			...
			if (!strcmp(exec_line_args[2], "login")) {
				...
				(exec_line_args_len == 7 ? AAA_AUTH_RADIUS_LOCAL : AAA_AUTH_RADIUS), 1, FILE_PAM_GENERIC)) {
					printf("%% Not possible to execute command with success\n");
					free_args_din(&exec_line_args);
					return;
				}
				...
				if (!conf_pam_mode(cish_cfg, 
				(exec_line_args_len == 7 ? AAA_AUTH_RADIUS_LOCAL : AAA_AUTH_RADIUS), 1, FILE_PAM_PPP)) {
					printf("%% Not possible to execute command with success\n");
					free_args_din(&exec_line_args);
					return;
				}
			...
			if (!strcmp(exec_line_args[2], "login")) {
				if(!conf_pam_mode(cish_cfg, 
				(exec_line_args_len == 7 ? AAA_AUTH_TACACS_LOCAL : AAA_AUTH_TACACS), 1, FILE_PAM_GENERIC)) {
				printf("%% Not possible to execute command with success\n");
				free_args_din(&exec_line_args);
				return;
				}
				...
				if(!conf_pam_mode(cish_cfg, 
				(exec_line_args_len == 7 ? AAA_AUTH_TACACS_LOCAL : AAA_AUTH_TACACS), 1, FILE_PAM_PPP)) {
				printf("%% Not possible to execute command with success\n");
				free_args_din(&exec_line_args);
				return;
				}
	...
	{	
		...
		if (!strcmp(exec_line_args[0], "no")) {
			if(!strcmp(exec_line_args[4], "1")) {
				if(!conf_pam_mode(cish_cfg, AAA_ACCT_TACACS_NO_CMD_1, 1, FILE_PAM_GENERIC)) {
					printf("%% Not possible to execute command with success\n");
					free_args_din(&exec_line_args);
					return;
				}
			} else if (!strcmp(exec_line_args[4], "15")) {
				if(!conf_pam_mode(cish_cfg, AAA_ACCT_TACACS_NO_CMD_15, 1, FILE_PAM_GENERIC)) {
					printf("%% Not possible to execute command with success\n");
					free_args_din(&exec_line_args);
					return;
				}
			...
			if(!strcmp(exec_line_args[3], "1")) {
				if(!conf_pam_mode(cish_cfg, AAA_ACCT_TACACS_CMD_1, 1, FILE_PAM_GENERIC)) {
					printf("%% Not possible to execute command with success\n");
					free_args_din(&exec_line_args);
					return;
				}
			} else if (!strcmp(exec_line_args[3], "15")) {
				if(!conf_pam_mode(cish_cfg, AAA_ACCT_TACACS_CMD_15, 1, FILE_PAM_GENERIC)) {
					printf("%% Not possible to execute command with success\n");
					free_args_din(&exec_line_args);
					return;
				}
	...
	if((exec_line_args_len = parse_args_din((char *) cmd, &exec_line_args)))
	{
		...
		if(!strcmp(exec_line_args[0], "no") || !strcmp(exec_line_args[4], "none")) {
			...
			if(!strcmp(exec_line_args[5], "tacacs+")) {
				...
				fclose(server);
				if(!conf_pam_mode(cish_cfg, (exec_line_args_len == 7 ? 
							AAA_AUTHOR_TACACS_LOCAL : AAA_AUTHOR_TACACS), 1, FILE_PAM_GENERIC)) {
					printf("%% Not possible to execute command with success\n");
					free_args_din(&exec_line_args);
					return;
				}
...
void add_user(const char *cmd) /* aaa username <user> password [hash] <pass> */ /* tinylogin */
{
	...
	system(buffer);
	if (args->argc == 6) snprintf(buffer, 255, "/bin/adduser %s -c '%s' >/dev/null 2>/dev/null", args->argv[2], args->argv[5]);
	else snprintf(buffer, 255, "/bin/adduser %s -p '%s' >/dev/null 2>/dev/null", args->argv[2], args->argv[4]);
	system(buffer);
	destroy_args(args);
}
...
void add_radiusserver(const char *cmd) /* radius-server host <ipaddr> [key <secret> [timeout <1-1000>]] */
{
	...
	{
		if (cish_cfg->radius[i].ip_addr[0] == 0 || !strncmp(cish_cfg->radius[i].ip_addr, args->argv[2], 16))
		{
			strncpy(cish_cfg->radius[i].ip_addr, args->argv[2], 16);
			if (args->argc >= 5) strncpy(cish_cfg->radius[i].authkey, args->argv[4], MAX_SERVER_AUTH_KEY);
				else strcpy(cish_cfg->radius[i].authkey, "");
			if (args->argc == 7) cish_cfg->radius[i].timeout=atoi(args->argv[6]);
				else cish_cfg->radius[i].timeout=0;
			break;
		}
	}
	if (i == MAX_SERVERS)
		...
		for(i=0; i < MAX_SERVERS; i++)
		{
			...
			{	/* server[:port] secret [timeout] */
				if(cish_cfg->radius[i].authkey[0])
					fprintf(server, "%s\t%s\t%d\n", cish_cfg->radius[i].ip_addr, cish_cfg->radius[i].authkey, cish_cfg->radius[i].timeout);
				else
...
void del_radiusserver(const char *cmd) /* no radius-server [host <ipaddr>] */
{
	...
	for (i=0; i < MAX_SERVERS; i++)
	{
		if (args->argc == 4)
		{
			...
			else cish_cfg->radius[i].ip_addr[0]=0;
	}
...
void add_tacacsserver(const char *cmd) /* tacacs-server host <ipaddr> key <secret> [timeout <1-1000>] */
{
	...
	for (i=0; i < MAX_SERVERS; i++)
	{
		if (cish_cfg->tacacs[i].ip_addr[0] == 0 || !strncmp(cish_cfg->tacacs[i].ip_addr, args->argv[2], 16))
		{
			strncpy(cish_cfg->tacacs[i].ip_addr, args->argv[2], 16);
			if (args->argc >= 5) strncpy(cish_cfg->tacacs[i].authkey, args->argv[4], MAX_SERVER_AUTH_KEY);
				else strcpy(cish_cfg->tacacs[i].authkey, "");
			if (args->argc == 7) cish_cfg->tacacs[i].timeout=atoi(args->argv[6]);
				else cish_cfg->tacacs[i].timeout=0;
			break;
		}
	}
	if (i == MAX_SERVERS)
		...
		for(i=0; i < MAX_SERVERS; i++)
		{
			...
			{	/* server[:port] secret [timeout] */
				if(cish_cfg->tacacs[i].authkey[0])
					fprintf(server, "%s\t%s\t%d\n", cish_cfg->tacacs[i].ip_addr, cish_cfg->tacacs[i].authkey, cish_cfg->tacacs[i].timeout);
				else
...
void del_tacacsserver(const char *cmd) /* no tacacs-server [host <ipaddr>] */
{
	...
	for (i=0; i < MAX_SERVERS; i++)
	{
		if (args->argc == 4)
		{
			...
			else cish_cfg->tacacs[i].ip_addr[0]=0;
	}
	if (i == MAX_SERVERS && args->argc == 4)
	{
		printf("%% Server not found!\n");
