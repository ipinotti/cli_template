...
static void print_mangle(const char *action, const char *dscp, const char *mark, const char *proto, const char *src,
			 const char *dst, const char *sports, const char *dports,
			 char *mangle, FILE *out, int conf_format, int mc, char *flags,
			 char *tos, char *dscp_match, char *state, char *icmp_type, char *icmp_type_code)
{
	char src_ports[32];
	char dst_ports[32];
	char *_src;
	...
	if (conf_format) pfprintf(out, "mark-rule ");
	if (conf_format) pfprintf (out, "%s ", mangle);
		else pfprintf (out, "    ");
	if (strcmp(action, "DSCP") == 0 && dscp)
	{
		dscp_class=dscp_to_name(strtol(dscp, NULL, 16));
		if (dscp_class) pfprintf(out, "dscp class %s ", dscp_class);
			else pfprintf(out, "dscp %d ", strtol(dscp, NULL, 16));
	}
	else if (strcmp(action, "MARK") == 0)
	{
		pfprintf (out, "mark ");
		if (mark) pfprintf(out, "%d ", strtol(mark, NULL, 16));
	}
		else pfprintf (out, "???? ");
	...
	if (dscp_match)
	{
		...
		if (dscp_class) pfprintf(out, "dscp class %s ", dscp_class);
			else pfprintf(out, "dscp %d ", strtol(dscp_match, NULL, 16));
	}
	if (state)
	{
...
void dump_mangle(char *xmangle, FILE *F, int conf_format)
{
	...
	strcpy(iptline, "/bin/iptables -t mangle -L -nv");
	...
	while (!feof (ipc))
	{
		...
		if (strncmp(buf, "Chain ", 6) == 0)
		{
			...
			trimcolumn(buf+6);
			strncpy(mangle, buf+6, 100); mangle[100]=0;
			manglep=0;
		}
		else if (strncmp(buf, " pkts", 5) != 0)
		{
			if ((strlen(buf)) && ((xmangle == NULL) || (strcmp(xmangle, mangle) == 0)))
			{
				arglist	*args;
				char *p;
				...
				if ((mark=strstr(buf, "MARK xset 0x"))) mark += 12;
				if ((dscp=strstr(buf, "DSCP set 0x"))) dscp += 11;
				if ((dscp_match=strstr(buf, "DSCP match 0x"))) dscp_match += 13;
				...
				if (mark) trimcolumn(mark);
				if (dscp) trimcolumn(dscp);
				if (dscp_match) trimcolumn(dscp_match);
				if (state) trimcolumn(state);
				if (icmp_type) trimcolumn(icmp_type);
				if (icmp_type_code) trimcolumn(icmp_type_code);

				if ((strcmp(type, "DSCP") == 0) ||
					(strcmp(type, "MARK") == 0))
				{
					if (strcmp(mangle, "INPUT") != 0 && strcmp(mangle, "PREROUTING") != 0 && strcmp(mangle, "OUTPUT") != 0 && strcmp(mangle, "POSTROUTING") != 0) /* filter CHAINs */
					{
						if ((!manglep) && (!conf_format))
						{
							pfprintf(F, "MARK rule %s\n", mangle);
						}
						manglep=1;
						mcount=buf;
						if (!conf_format)
						{
							while (*mcount == ' ') ++mcount; /* pkts */
						}
						print_mangle(type,dscp,mark,prot,source,dest,sports,dports,mangle,F,conf_format,atoi(mcount),flags,tos,dscp_match,state,icmp_type,icmp_type_code);
					}
				}
				else
				{
					if (!conf_format)
					{
						...
						{
							if (strcmp(input, "*")) pfprintf(F, "interface %s in mark-rule %s\n", input, type);
							if (strcmp(output, "*")) pfprintf(F, "interface %s out mark-rule %s\n", output, type);
						}
					}
				}
				destroy_args(args);
...
typedef enum {mangle_dscp,mangle_dscp_class,mangle_mark} mangle_action;
typedef enum {ip=0, icmp=1, tcp=6, udp=17} proto;
typedef enum {add_mangle,insert_mangle,remove_mangle} mangle_mode;
typedef enum {st_established=0x1, st_new=0x2, st_related=0x4} mangle_state;
void do_mangle(const char *cmdline)
{
	...
	{
		if (strcmp(args->argv[crsr+1], "class") == 0) { crsr+=2; action = mangle_dscp_class; }
			else { crsr+=1; action = mangle_dscp; }
	}
	...
	switch (mode)
	{
		case insert_mangle: strcat(cmd, "-I "); break;
		case remove_mangle: strcat(cmd, "-D "); break;
	...
	if (dscp_class)
	{
		sprintf(cmd+strlen(cmd), "-m dscp --dscp-class %s ", dscp_class);
	}
	if (state)
	{
		int comma=0;
	...
	switch (action)
	{
		case mangle_dscp: strcat(cmd, "-j DSCP --set-dscp "); break;
		case mangle_dscp_class: strcat(cmd, "-j DSCP --set-dscp-class "); break;
		case mangle_mark: strcat(cmd, "-j MARK --set-mark "); break;
		default:
		...
		if((f = fopen(TMP_CFG_FILE, "w+")))
		{
			...
			while(fgets((char *)buf, 511, f))
			{
				if((n = parse_args_din((char *)buf, &argl)) > 3)
				{
					...
					{
						if(!strcmp(args->argv[0], "mark-rule"))
						{
...
#ifdef CHECK_IPTABLES_EXEC
	/* Temporary file for errors */
	strcpy(iptline, "/bin/iptables -t mangle -L -n");
	connect_error_file(iptline, randomfile); /* Temporary file for errors */
	F=popen(iptline, "r");
#else
	...
	while(!feof(F))
	{
		...
		else if ((in_chain) && (strncmp(buf, " pkts", 5) != 0) && (strlen(buf) > 40))
		{
			...
			if (((iface_in == NULL) || (strcmp(iface_in_, iface_in) == 0)) &&
			    ...
			    ((mangle == NULL) || (strcmp(target, mangle) == 0)))
			{
				mangle_exists=1;
				destroy_args(args);
				break;
...
{
	typedef enum {chain_in, chain_out, chain_other} mangle_chain;
	FILE *F;
	char buf[256];
	...
	while (!feof(F))
	{
		...
		if (strncmp(buf, "Chain ", 6) == 0)
		{
			if (strncmp(buf+6, "INPUT", 5) == 0) chain=chain_in;
		...
		else if ((strncmp(buf, " pkts", 5) != 0) && (strlen(buf) > 40))
		{
			...
			{
				mangle_in=target;
				strncpy(in_mangle, mangle_in, 100); in_mangle[100]=0;
			}
			if ((chain == chain_out) && (strcmp(iface, iface_out_) == 0))
			{
				mangle_out=target;
				strncpy(out_mangle, mangle_out, 100); out_mangle[100]=0;
			}
			if (mangle_in && mangle_out) break;
			destroy_args(args);
		}
	}
	pclose(F);
...
int clean_iface_mangle_rules(char *iface)
{
	...
	while (!feof(F))
	{
		...
		else if ((strncmp(buf, " pkts", 5) != 0)&&(strlen(buf) > 40))
		{
			...
			{
				sprintf(cmd, "/bin/iptables -t mangle -D %s -i %s -j %s", chain, iface_in_, target);
#ifdef CHECK_IPTABLES_EXEC
				connect_error_file(cmd, randomfile); /* Temporary file for errors */
#endif
				DEBUG_CMD(cmd);
			...
			{
				sprintf(cmd, "/bin/iptables -t mangle -D %s -o %s -j %s", chain, iface_out_, target);
#ifdef CHECK_IPTABLES_EXEC
				connect_error_file(cmd, randomfile); /* Temporary file for errors */
#endif
				DEBUG_CMD(cmd);
...
void interface_mangle(const char *cmdline)
{
	...
	if ((chain == chain_in) &&
	    (matched_mangle_exists(0, dev, 0, "INPUT") ||
	...
	if ((chain == chain_out) &&
	    (matched_mangle_exists(0, 0, dev, "OUTPUT") ||
	     matched_mangle_exists(0, 0, dev, "POSTROUTING")))
	{
	...
	if (chain == chain_in)
	{
		sprintf(buf, "/bin/iptables -t mangle -A INPUT -i %s -j %s", dev, listno);
#ifdef CHECK_IPTABLES_EXEC
		connect_error_file(buf, randomfile); /* Temporary file for errors */
#endif
		DEBUG_CMD(buf);
		...
		sprintf(buf, "/bin/iptables -t mangle -A PREROUTING -i %s -j %s", dev, listno);
#ifdef CHECK_IPTABLES_EXEC
		connect_error_file(buf, randomfile); /* Temporary file for errors */
#endif
		DEBUG_CMD(buf);
	...
	{
		sprintf(buf, "/bin/iptables -t mangle -A OUTPUT -o %s -j %s", dev, listno);
#ifdef CHECK_IPTABLES_EXEC
		connect_error_file(buf, randomfile); /* Temporary file for errors */
#endif
		DEBUG_CMD(buf);
		...
		sprintf(buf, "/bin/iptables -t mangle -A POSTROUTING -o %s -j %s", dev, listno);
#ifdef CHECK_IPTABLES_EXEC
		connect_error_file(buf, randomfile); /* Temporary file for errors */
#endif
		DEBUG_CMD(buf);
...
void interface_no_mangle(const char *cmdline)
{
	...
	if ((chain == chain_in) || (chain == chain_both))
	{
		...
		{
			sprintf(buf, "/bin/iptables -t mangle -D INPUT -i %s -j %s", dev, listno);
#ifdef CHECK_IPTABLES_EXEC
			connect_error_file(buf, randomfile); /* Temporary file for errors */
#endif
			DEBUG_CMD(buf);
		...
		{
			sprintf(buf, "/bin/iptables -t mangle -D PREROUTING -i %s -j %s", dev, listno);
#ifdef CHECK_IPTABLES_EXEC
			connect_error_file(buf, randomfile); /* Temporary file for errors */
#endif
			DEBUG_CMD(buf);
	...
	if ((chain == chain_out) || (chain == chain_both))
	{
		...
		{
			sprintf(buf, "/bin/iptables -t mangle -D OUTPUT -o %s -j %s", dev, listno);
#ifdef CHECK_IPTABLES_EXEC
			connect_error_file(buf, randomfile); /* Temporary file for errors */
#endif
			DEBUG_CMD(buf);
		...
		{
			sprintf(buf, "/bin/iptables -t mangle -D POSTROUTING -o %s -j %s", dev, listno);
#ifdef CHECK_IPTABLES_EXEC
			connect_error_file(buf, randomfile); /* Temporary file for errors */
#endif
			DEBUG_CMD(buf);
